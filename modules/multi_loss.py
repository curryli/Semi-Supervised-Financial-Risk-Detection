...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.class_freq = torch.from_numpy(np.asarray(class_freq)).float().to(device)
        ...
        self.freq_inv = torch.ones(self.class_freq.shape).to(device) / self.class_freq
    ...
    def forward(self,
        ...
        weight = self.reweight_functions(label, self.device)
    ...
    def reweight_functions(self, label, device):
        if self.reweight_func is None:
            ...
            weight = self.CB_weight(label.float(), device)
        else:
            return None
    ...
    def CB_weight(self, gt_labels, device):
